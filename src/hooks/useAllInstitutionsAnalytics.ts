...
export function useAllInstitutionsAnalytics() {
  return useQuery({
    ...
    queryFn: async (): Promise<InstitutionEngagement[]> => {
      ...
      // Build per-institution top-student completion %
      const allAssignmentIds = (courseAssignments || []).map(ca => ca.id);
      const completionRateByInstitution = new Map<string, number>();

      if (allAssignmentIds.length > 0) {
        const ctxMap = await buildSessionCompletionContexts(allAssignmentIds);

        // Group assignments by class, students by class, by institution
        const assignmentsByClass = new Map<string, string[]>();
        const classToInstitution = new Map<string, string>();
        (courseAssignments || []).forEach(ca => {
          const arr = assignmentsByClass.get(ca.class_id) || [];
          arr.push(ca.id);
          assignmentsByClass.set(ca.class_id, arr);
          if (ca.institution_id) classToInstitution.set(ca.class_id, ca.institution_id);
        });
        ...
        const topPctsByInstitution = new Map<string, number[]>();
        for (const [cid, sids] of studentsByClass.entries()) {
          const aids = assignmentsByClass.get(cid) || [];
          if (aids.length === 0) continue;
          const ctxArr = aids.map(aid => ctxMap.get(aid)).filter(Boolean) as any[];
          if (ctxArr.length === 0) continue;
          ...
          const inst = classToInstitution.get(cid);
          if (!inst) continue;
          const arr = topPctsByInstitution.get(inst) || [];
          arr.push(Math.max(...pcts));
          topPctsByInstitution.set(inst, arr);
        }

        topPctsByInstitution.forEach((pcts, inst) => {
          completionRateByInstitution.set(inst, Math.round(Math.max(...pcts) * 10) / 10);
        });
      }
