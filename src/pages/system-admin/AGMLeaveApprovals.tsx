...
export default function AGMLeaveApprovals() {
  ...
  const loadApplications = () => {
    const apps = getLeaveApplicationsByStage('agm_pending');
  ...
  const handleApprove = () => {
    ...
    try {
      approveLeaveApplicationAGM(selectedApplication.id, user.name, comments || undefined);
      toast.success('Leave application fully approved');
      loadApplications();
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div className="space-y-2 flex-1">
                        ...
                        {app.approved_by_manager && (
                          <div className="bg-green-50 p-2 rounded text-sm">
                            <p className="text-xs text-muted-foreground">Manager Approval:</p>
                            <p className="font-medium">{app.approved_by_manager}</p>
                            {app.manager_comments && (
                              <p className="text-xs mt-1">{app.manager_comments}</p>
                            )}
                          </div>
                        )}
                      </div>
