  ...
  const handleSubmit = async () => {
    ...
    try {
      await createCreditDebitNote({
        ...
        customer_name: invoice?.to_company_name,
        customer_address: [
          ...
          .filter(Boolean)
          .join(', '),
        customer_gstin: invoice?.to_company_gstin,
        reason: finalReason,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {invoice
              ? `Issue credit note against Invoice ${invoice.invoice_number}`
              : 'Create a standalone credit note'}
          </DialogDescription>
        </DialogHeader>

        <div className="space-y-6 py-4">
          {/* Customer Info (if from invoice) */}
          {invoice && (
